...
# for index in range(starting_instant, starting_instant + 10):
    ...
    mu_val_i = np.where(A_i_minus1 !=0, b_i_minus1/A_i_minus1, np.inf)
    ...
    # for mu_iter in enumerate(mu_val_i.flatten()):
    #     satisfy_all_i = True
    #     for j in range(len(A_i_minus1)):
    #         if not (A_i_minus1[j] * mu_iter <= b_i_minus1[j] + 1e-6):
    #             satisfy_all_i = False
    #             break
        
    #     if satisfy_all_i:
    #         mu_i.append(max(mu_iter, 0))  # Ensure non-negative mu
    
    valid_mu = remove_duplicates(valid_mu, epsilon=1e-3)   # This removes values similar to each other
    valid_mu = np.sort(valid_mu)
    mu_i = remove_duplicates(mu_i, 1e-3)
    mu_i = np.sort(mu_i)

  
    print(f"Iteration: {index}: mu ∈ [{valid_mu[0]:.4f}, {valid_mu[1]:.4f}] ")
    # print(f"Iteration: {index}: mu ∈ [{mu_i[0]:.4f}, {mu_i[1]:.4f}] ")
    # A_i_minus2 = []
    # b_i_minus2 = []
