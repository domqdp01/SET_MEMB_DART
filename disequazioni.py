...
z_1 = np.array([[vx[index - 1]], [vy[index - 1]], [w[index - 1]]])  # dim: (6,1)

# LATERAL DYNAMICS
delta0 = steer_angle(steering_input[index - 2])
alpha_f0, alpha_r0 = slip_angles(vx[index - 2], vy[index - 2], w[index - 2], delta0)
F_y_f_0, F_y_r_0 = lateral_tire_forces(alpha_f0, alpha_r0)

# LONGITUDINAL DYNAMICS
# F_rolling_0 = rolling_friction(vx[0])

F_m_0 = motor_force(tau[index - 2], vx[index - 2])
# F_fric_0 = F_friction_due_to_steering(delta0, vx[0])
F_fric_0 = friction(vx[index - 2])
...
# SET UP MATRICES
F_0 = np.array([
    1/m * (F_x_0_r + F_x_0_f * np.cos(delta0)) + w[index - 2] * vy[index - 2],
    1/m * (F_x_0_f * np.sin(delta0)) - w[index - 2] * vx[index - 2],
